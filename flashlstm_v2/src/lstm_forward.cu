...
namespace {
    ...
    __global__ void QuantizeCheckpointKernel(
        const float *checkpoint_src, // (N, 2, B, H) float
        __half *checkpoint_dst, // (N, 2, B, H) half
        float *column_scale_dst, // (N * B,) scale factors
        size_t checkpoints,
        size_t batch_size,
        size_t hidden_size
    ) {
        ...
        const float *h_src = checkpoint_src + checkpoint_offset + batch_idx * hidden_size;
        const float *c_src = checkpoint_src + checkpoint_offset + bh + batch_idx * hidden_size;
        __half *h_dst = checkpoint_dst + checkpoint_offset + batch_idx * hidden_size;
        __half *c_dst = checkpoint_dst + checkpoint_offset + bh + batch_idx * hidden_size;

        float local_max = 0.0f;
        for (size_t idx = threadIdx.x; idx < hidden_size; idx += blockDim.x) {
            local_max = fmaxf(local_max, fabsf(h_src[idx]));
            local_max = fmaxf(local_max, fabsf(c_src[idx]));
        }
        shared[threadIdx.x] = local_max;
        __syncthreads();
        ...
        for (size_t idx = threadIdx.x; idx < hidden_size; idx += blockDim.x) {
            float h_val = h_src[idx] * inv_scale;
            h_val = fminf(fmaxf(h_val, -kFp16SafeMax), kFp16SafeMax);
            h_dst[idx] = __float2half(h_val);
            float c_val = c_src[idx] * inv_scale;
            c_val = fminf(fmaxf(c_val, -kFp16SafeMax), kFp16SafeMax);
            c_dst[idx] = __float2half(c_val);
        }
    }
    ...
    void StreamingLstmForward(
        ...
        if ((gate_cache_host == nullptr) != (gate_cache_scale_host == nullptr)) {
            throw std::runtime_error(
                "StreamingLstmForward requires gate_cache_host and gate_cache_scale_host together");
        }
        ...
        HostRegistration gate_scale_registration;
        ...
        if (store_checkpoints) {
            ...
            gate_scale_registration.reset(
                gate_cache_scale_host,
                checkpoint_count * batch_size * sizeof(float),
                "cudaHostRegister checkpoint_scale_host"
            );
        }
        ...
        const size_t max_checkpoints_per_chunk = std::max<size_t>(
            1, (chunk_capacity + checkpoint_stride - 1) / checkpoint_stride + 1);
        const size_t checkpoint_entry_elements = 2 * bh_elements;
        DeviceBuffer<float> checkpoint_chunks[2];
        DeviceBuffer<__half> checkpoint_chunks_half[2];
        DeviceBuffer<float> checkpoint_scale_chunks[2];
        if (store_checkpoints) {
            AllocateDeviceBufferArray(checkpoint_chunks, max_checkpoints_per_chunk * checkpoint_entry_elements,
                                      "cudaMalloc checkpoint_chunk");
            AllocateDeviceBufferArray(checkpoint_chunks_half, max_checkpoints_per_chunk * checkpoint_entry_elements,
                                      "cudaMalloc checkpoint_chunk_half");
            AllocateDeviceBufferArray(checkpoint_scale_chunks, max_checkpoints_per_chunk * batch_size,
                                      "cudaMalloc checkpoint_chunk_scale");
        }
        ...
        while (chunk_idx < total_chunks) {
            ...
            if (store_checkpoints) {
                ...
                if (checkpoint_count_chunk > 0) {
                    ...
                    const size_t total_columns = checkpoint_count_chunk * batch_size;
                    if (total_columns > 0) {
                        const int quant_threads = 128;
                        const dim3 quant_grid(static_cast<unsigned int>(total_columns));
                        const size_t shared_bytes = quant_threads * sizeof(float);
                        QuantizeCheckpointKernel<<<quant_grid, quant_threads, shared_bytes, d2h_stream>>>(
                            checkpoint_chunks[slot].ptr,
                            checkpoint_chunks_half[slot].ptr,
                            checkpoint_scale_chunks[slot].ptr,
                            checkpoint_count_chunk,
                            batch_size,
                            hidden_size
                        );
                        CheckCuda(cudaGetLastError(), "QuantizeCheckpointKernel");
                    }
                    ...
                    float *scale_dst = gate_cache_scale_host + checkpoint_host_offsets[slot] * batch_size;
                    const size_t scale_count = checkpoint_count_chunk * batch_size;
                    CheckCuda(cudaMemcpyAsync(
                                  scale_dst,
                                  checkpoint_scale_chunks[slot].ptr,
                                  ...
                                  cudaMemcpyDeviceToHost,
                                  d2h_stream),
                              "copy checkpoint scale chunk");

                    CheckCuda(cudaEventRecord(checkpoint_copy_done[slot].evt, d2h_stream),
                              "record checkpoint_copy_done");
