...
class _LSTMFunction(Function):
    ...
    def forward(  # type: ignore[override]
        ...
        if x.dtype != torch.float16:
            raise ValueError(f"x must use dtype torch.float16, got {x.dtype}.")
    ...
    def backward(  # type: ignore[override]
        ...
        if grad_y is None:
            grad_y = torch.zeros_like(y_device)
        else:
...
def lstm(
    ...
    *,
    ...
    """
    Functional wrapper for the device-resident LSTM kernels.
    """
    ...
    y, gate_cache_h, gate_cache_c, hy, cy = outputs
